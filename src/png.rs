...
impl Png {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut bytes: Vec<u8> = Self::STANDARD_HEADER.to_vec();
        for chunk in self.chunks.iter() {
            let mut b = chunk.as_bytes();
            bytes.append(&mut b);
        }
        return bytes;
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        while i < len {
            let chunk_len = u32::from_be_bytes(value[i..(i + 4)].try_into()?) as usize;
            let chunk_bytes = value[i..i + 4 + 4 + chunk_len + 4].to_vec();
